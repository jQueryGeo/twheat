...
function initMap(center, zoom) {
  ...
  map = $("#map").geomap({
    ...
    bboxchange: function( e, geo ) {
      let state = deparam( window.location.hash.substr( 1 ) );
...
$( window ).on( 'hashchange', function( ) {
  ...
  if (state.l) {
    let loc = decodeURIComponent(state.l);
    $("#loc input").val(loc);
    title += " in " + loc;
  }
  ...
  $("#tweetButton").html('');
  setTimeout( function( ) {
    let twitterButtonHtml = '<a href="https://twitter.com/intent/tweet" class="twitter-share-button" data-count="vertical">Tweet</a>';
